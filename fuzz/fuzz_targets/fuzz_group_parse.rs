...
/// Field separators in a well-formed `/etc/group` line.
...
fuzz_target!(|data: &[u8]| {
    ...
    let Ok(entry) = text.parse::<GroupEntry>() else {
        return;
    };
    ...
    assert_eq!(
        ...
        SEPARATORS,
        "rendered group record has the wrong number of separators: {rendered:?}"
    );
